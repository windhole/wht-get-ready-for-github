...
書き込みは次のどれか 1 つを付けたときだけ行います。引数なしでは、先頭にバージョンを出したあと、ヘルプとプレビューを出します。

| フラグ | 動き |
|--------|------|
| （なし） | バージョン、ヘルプ、プレビュー（書き込みなし） |
| `--version` | バージョンだけ表示 |
| `--init` | 従来どおり。LICENSE は無ければ書く。公開範囲は都度確認 |
| `--doc` | LICENSE を書かない。private（確認なし） |
| `--dev` | LICENSE を書く。public（確認なし） |

`--init` / `--doc` / `--dev` / `--version` は同時にどれか 1 つだけです。
